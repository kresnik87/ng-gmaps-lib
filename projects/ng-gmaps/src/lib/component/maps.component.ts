...
import {MapRestriction, MouseEvent} from '../types';
...
@Component({
  selector: 'ksk-google-map',
  providers: [
    GoogleMapsCore, MarkerManager, InfoWindowManager, FitBoundsService
  ],
  styles: [`
    .map-container-inner {
      width: inherit;
...
export class KskMap implements OnChanges, OnInit, OnDestroy
{
  /**
   ...
   * If this option to `true`, the bounds get automatically computed from all elements that use the {@link FitBounds} directive.
   */
  ...
  constructor(private _elem: ElementRef, private _mapsCore: GoogleMapsCore, protected _fitBoundsService: FitBoundsService, private _zone: NgZone)
  {
  }
  ...
  ngOnDestroy()
  {
    ...
    this._mapsCore.clearInstanceListeners();
  ...
  triggerResize(recenter: boolean = true): Promise<void>
  {
    ...
    return new Promise<void>((resolve) =>
    {
      setTimeout(() =>
      {
        return this._mapsCore.triggerMapEvent('resize').then(() =>
        {
          if (recenter)
          {
  ...
  protected _updateBounds(bounds: google.maps.LatLngBounds | google.maps.LatLngBoundsLiteral)
  {
    ...
    if (this.usePanning)
    {
      this._mapsCore.panToBounds(bounds);
      return;
    }
    this._mapsCore.fitBounds(bounds);
  }

  private _initMapInstance(el: HTMLElement)
  {
    this._mapsCore.createMap(el, {
      ...
      .then(() => this._mapsCore.getNativeMap())
  ...
  private _updateMapOptionsChanges(changes: SimpleChanges)
  {
    ...
    let optionKeys =
      Object.keys(changes).filter(k => KskMap._mapOptionsAttributes.indexOf(k) !== -1);
    optionKeys.forEach((k) =>
    {
      options[k] = changes[k].currentValue;
    });
    this._mapsCore.setMapOptions(options);
  }
  ...
  private _setCenter()
  {
    ...
    if (this.usePanning)
    {
      this._mapsCore.panTo(newCenter);
    }
    else
    {
      this._mapsCore.setCenter(newCenter);
    }
  }
  ...
  {
    const s = this._mapsCore.subscribeToMapEvent<void>('center_changed').subscribe(() =>
    {
      this._mapsCore.getCenter().then((center: google.maps.LatLng) =>
      {
  ...
  {
    const s = this._mapsCore.subscribeToMapEvent<void>('bounds_changed').subscribe(() =>
    {
      this._mapsCore.getBounds().then(
  ...
  {
    const s = this._mapsCore.subscribeToMapEvent<void>('maptypeid_changed').subscribe(() =>
    {
      this._mapsCore.getMapTypeId().then(
        (mapTypeId: MapTypeId) =>
        {
  ...
  {
    const s = this._mapsCore.subscribeToMapEvent<void>('zoom_changed').subscribe(() =>
    {
      this._mapsCore.getZoom().then((z: number) =>
      {
        this.zoom = z;
  ...
  {
    const s = this._mapsCore.subscribeToMapEvent<void>('idle').subscribe(
      () =>
      {
  ...
  private _handleMapMouseEvents()
  {
    ...
    {
      const s = this._mapsCore.subscribeToMapEvent<{ latLng: google.maps.LatLng }>(e.name).subscribe(
